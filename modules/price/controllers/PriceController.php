...
class PriceController extends CmsController
{
	/////////////////////////////
	...
	public $title='<i class="fa fa-dollar"></i> Prices';
	public $subTitle='Admin Prices';
	...
	public function actionCreate()
	{
		...
		{
			$model->attributes=$_POST['PriceItems'];
			$last=PriceItems::model()->findAll();
	...
	{
		$model=new PriceItems('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['PriceItems']))
			$model->attributes=$_GET['PriceItems'];
	...
	public function actionOrder()
	{
        ...
        for($i = 0 ; $i < $cant ; $i++){
	        ...
	        $ini = $ini + 1 ;
	        $model=PriceItems::model()->findByPk($datos['id']);
	 ...
	 * @return PriceItems the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=PriceItems::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param PriceItems $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='price-items-form')
		{
