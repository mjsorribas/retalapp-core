...
<?php $this->widget('zii.widgets.grid.CGridView', array(
	...
	'filter'=>$model,    'afterAjaxUpdate'=>"js:function(){
        $('#customers-items-grid tbody').sortable({ opacity: 0.00001 });
        $('#customers-items-grid tbody').sortable({
            update: function() {
                var that = $(this);
	        ...
	        if(result) {
	            $.ajax({
	                ...
	                success:function (data) {
	                    $.fn.yiiGridView.update('customers-items-grid');
	                }
	            });
	        }
	    });
