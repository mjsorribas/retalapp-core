...
<?php $this->widget('zii.widgets.grid.CGridView', array(
	...
	'filter'=>$model,    'afterAjaxUpdate'=>"js:function(){
        $('#clients-items-grid tbody').sortable({ opacity: 0.00001 });
        $('#clients-items-grid tbody').sortable({
            update: function() {
                var that = $(this);
	...
	'columns'=>array(
		...
		array(
			...
			'filter'=>false,
			'type'=>'raw',
			'value'=>'"<span class=\"text-muted\">".substr($data->image_hover,0,50)."...</span>"',
			'value'=>'CHtml::image(Yii::app()->request->baseUrl."/uploads/".$data->image_hover,"",array("class"=>"img-responsive img-thumbnail","style"=>"max-width:100px"))',
		),
		/*array(
			'class'=>'CButtonColumn',
		),*/
	        ...
	        if(result) {
	            $.ajax({
	                ...
	                success:function (data) {
	                    $.fn.yiiGridView.update('clients-items-grid');
	                },
