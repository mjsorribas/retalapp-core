...
<?php $this->widget('zii.widgets.grid.CGridView', array(
	'id'=>'landing-elements-slider-grid',
	...
	'filter'=>$model,    'afterAjaxUpdate'=>"js:function(){
        $('#landing-elements-slider-grid tbody').sortable({ opacity: 0.00001 });
        $('#landing-elements-slider-grid tbody').sortable({
            update: function() {
                var that = $(this);
	...
	'columns'=>array(
		...
		array(
			'name'=>'link',
			'type'=>'raw',
			'value'=>'CHtml::link("<i class=\"fa fa-external-link\"></i> ".strtr($data->link,array("http://"=>"","https://"=>"")),$data->link,array("target"=>"_blank"))',
		),
		*/
		/*array(
			'class'=>'CButtonColumn',
	        ...
	        if(result) {
	            $.ajax({
	                ...
	                success:function (data) {
	                    $.fn.yiiGridView.update('landing-elements-slider-grid');
	                },
