...
<?php $this->widget('zii.widgets.grid.CGridView', array(
	...
	'filter'=>$model,    'afterAjaxUpdate'=>"js:function(){
        $('#blog-posts-grid tbody').sortable({ opacity: 0.00001 });
        $('#blog-posts-grid tbody').sortable({
            update: function() {
                var that = $(this);
	...
	'columns'=>array(
		...
		array(
			'name'=>'youtube',
			'type'=>'raw',
			'value'=>'CHtml::link("<i class=\"fa fa-external-link\"></i> ".strtr($data->youtube,array("http://"=>"","https://"=>"")),$data->youtube,array("target"=>"_blank"))',
		),
		array(
			'name'=>'vimeo',
			'type'=>'raw',
			'value'=>'CHtml::link("<i class=\"fa fa-external-link\"></i> ".strtr($data->vimeo,array("http://"=>"","https://"=>"")),$data->vimeo,array("target"=>"_blank"))',
		),
		array(
			'name'=>'image',
			'filter'=>false,
