...
<?php $this->widget('zii.widgets.grid.CGridView', array(
	...
	'filter'=>$model,    'afterAjaxUpdate'=>"js:function(){
        $('#users-location-states-grid tbody').sortable({ opacity: 0.00001 });
        $('#users-location-states-grid tbody').sortable({
            update: function() {
                var that = $(this);
	...
	'columns'=>array(
		...
		array(
			'name'=>'users_location_countries_id',
			'filter'=> UsersLocationCountries::listData(),
			'type'=>'raw',
			'value'=>'$data->users_location_countries_id',
	        ...
	        if(result) {
	            $.ajax({
	                ...
	                success:function (data) {
	                    $.fn.yiiGridView.update('users-location-states-grid');
	                },
