...
<?php $this->widget('zii.widgets.grid.CGridView', array(
	'id'=>'users-location-countries-grid',
	...
	'filter'=>$model,    'afterAjaxUpdate'=>"js:function(){
        $('#users-location-countries-grid tbody').sortable({ opacity: 0.00001 });
        $('#users-location-countries-grid tbody').sortable({
            update: function() {
                var that = $(this);
	        ...
	        if(result) {
	            $.ajax({
	                ...
	                success:function (data) {
	                    $.fn.yiiGridView.update('users-location-countries-grid');
	                },
