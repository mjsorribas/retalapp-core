...
class Api_usersController extends JsonController
{
    ...
    {
        $module=$this->module;
        $app->notFound(function () use ($app,$r) {
            ...
            $res
                ->status(404)
                ->json($req, Yii::t('app','The resourse requested does not exist '.$r));
            $app->stop();
        ...
        $cb = function () use ($module, $app) {
            return function() use ($module, $app) {
                if(!r()->user->checkToken($module->getAllowPermissoms())) {
                    $app->response
                        ->status(403)
        ...
        $app->get("$r/me", $cb(), function () use ($app) {
            ...
            $model=Users::model()->findByPk(r()->user->userToken->id);
            if($model===null) {
                $res
                    ->status(404)
        ...
        $app->post("$r/login",function () use ($module,$app) {
            ...
            if(!$appForm->validate()) {
                ...
                $app2=Apps::model()->find('client_id=? AND client_secret=?',
                    array($req->params('client_id'),$req->params('client_secret')));
            ...
            if($model->validate() and ($userObject=$identity->authenticateToken())!==false)
            {
                ...
                $modelArray['token']=$userObject->getAccessToken($app2);
                $res
                    ->status(200)
        ...
        $app->post("$r/update_profile", function () use ($app) {
            $req=$app->request;
            $res=$app->response;
            ...
            if($model->save())
            {
                ...
                $res
                    ->status(200)
                    ->json($model,Yii::t('app','has been updated successfully')."");
            } else {
                $res
                    ->status(422)
                    ->json($model->getErrors(),Yii::t('app','Form validation errors'),"unprocessable_entity");
